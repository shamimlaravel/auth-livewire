<div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">{{ __('User Management') }}</h1>
            <p class="mt-2 text-sm text-gray-600">{{ __('Manage users, roles, and account status.') }}</p>
        </div>
        ...
        <div class="flex flex-wrap gap-4">
            <div class="flex-1">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Search by name or email...') }}"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500">
            </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ __('2FA') }}</th>
                ...
                @foreach ($users as $user)
                    <tr class="hover:bg-gray-50">
                        @if ($editingUserId === $user->id)
                            <td class="px-6 py-4 text-sm font-medium text-gray-900" colspan="6">
                                <form wire:submit="saveUser" class="flex flex-wrap items-end gap-4">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500">{{ __('Role') }}</label>
                                        <select wire:model="editingRole" class="mt-1 rounded-lg border border-gray-300 px-3 py-2 text-sm">
                                            @foreach (Spatie\Permission\Models\Role::all() as $role)
                                                <option value="{{ $role->name }}">{{ $role->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="editingStatus" class="mt-1 rounded-lg border border-gray-300 px-3 py-2 text-sm">
                                            <option value="active">{{ __('Active') }}</option>
                                            <option value="suspended">{{ __('Suspended') }}</option>
                                            <option value="banned">{{ __('Banned') }}</option>
                                        </select>

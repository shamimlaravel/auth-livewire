...
<body class="flex min-h-screen items-center justify-center bg-surface-50 px-4">
    <div class="w-full max-w-md text-center">
        <h1 class="text-7xl font-bold text-brand-500">403</h1>
        <h2 class="mt-4 text-2xl font-semibold text-surface-900">{{ __('Forbidden') }}</h2>
        <p class="mt-2 text-surface-500">{{ __($exception->getMessage() ?: 'You do not have permission to access this resource.') }}</p>

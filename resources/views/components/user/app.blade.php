    ...
    <div class="flex min-h-screen">
        ...
        <aside class="fixed left-0 top-0 z-30 flex h-full w-64 flex-col bg-surface-900">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto p-4">
                <a href="{{ route('dashboard') }}" wire:navigate
                   class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-surface-300 hover:bg-white/10 hover:text-white {{ request()->routeIs('dashboard') ? 'bg-brand-500/20 text-white' : '' }}">
                    {{ __('Dashboard') }}
                </a>
                <a href="{{ route('settings') }}" wire:navigate
                   class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-surface-300 hover:bg-white/10 hover:text-white {{ request()->routeIs('settings') ? 'bg-brand-500/20 text-white' : '' }}">
                    {{ __('Account Settings') }}
                </a>
            </nav>
        ...
        <div class="flex flex-1 flex-col pl-64">
            <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-surface-200 bg-white px-6">
                <h1 class="text-lg font-semibold text-surface-900">{{ $header ?? __('Dashboard') }}</h1>
